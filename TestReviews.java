/**
 * authors: Avruti Srivastava, Sohil Jain
 */
...
public class TestReviews {
	...
	public static void main(String[] args) throws ParseException, IOException, JSONException {
		...
		IndexReader reader = DirectoryReader
				.open(FSDirectory.open(Paths.get("C:\\Users\\avruti\\Desktop\\yelp_indexrev")));
		...
		String line = null;
		String fileName = "C:\\Users\\avruti\\Desktop\\reviewtest1.txt";
		...
		// calculating result for each business ID
		one: for (int i = 0; i < Vectors.size(); i++) {

			// System.out.println(i);
			String queryString = Vectors.get(i).get(2);
			String Labels = Vectors.get(i).get(1);
			CustomAnalyzer analyzer = CustomAnalyzer.builder(Paths.get("C:\\Users\\avruti\\Desktop"))
					.withTokenizer("standard").addTokenFilter("standard").addTokenFilter("lowercase")
					.addTokenFilter("stop", "ignoreCase", "false").addTokenFilter("keepWord", "words",
							"C:\\Users\\avruti\\Desktop\\keepwords.txt", "ignoreCase", "false")
					.addTokenFilter("porterstem").addTokenFilter("removeduplicates").build();
			// StandardAnalyzer analyzer=new StandardAnalyzer();
			QueryParser parser = new QueryParser("review", analyzer);
			Query query = parser.parse(QueryParser.escape(queryString));
			BooleanQuery.setMaxClauseCount(Integer.MAX_VALUE);
			TopScoreDocCollector collector = TopScoreDocCollector.create(7);
